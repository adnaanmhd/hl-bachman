...
| frozen     | No run > 30 consecutive native-frame-equivalents with near-zero motion (trans < 0.1 px, rot < 0.001°).                         | Derived from motion samples. |
...
- Conf-based metrics: max confidence across *qualifying* (originally pass) frames in the merged window. Absorbed fail-frames never contribute.
